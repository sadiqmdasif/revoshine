...
class Revo_authentication
{
	...
	{
		if (!$this->_temporary_test()) {
			$error = new WP_Error('revo_rest_authentication_error', __('Consumer key is invalid.', 'woocommerce'), array('status' => 401));
	...
	private function _temporary_test()
	{
		global $wpdb;
		...
		if (is_null($user) || $user->permissions !== 'read_write') {
			return false;
		}
